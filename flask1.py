...
mydb = mysql.connector.connect(host="localhost",user="root",password="Priyanka@14",database="dbo")
...
@app.route('/<email>')
def renter(email):
    query_property='select * from property where propertyid >=1'
...
def bookings(data):
    ...
    # data=eval(data)
    query_bookings='select bookingrecords.emailID,BookingID,property_type, location, city,address,price,description,availability,crime_rate,nearby_schools from bookingrecords inner join property on property.propertyID=bookingrecords.propertyID having emailID=%s'
...
def submit_ad(email):
    if(request.method=="POST"):
        ...
        query_house_apartment='insert into property(emailID,property_type,location,city,address,price,description,availability,crime_rate,nearby_schools) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
        mycursor.execute(query_house_apartment,(email,property_type,location,city,address,price,description,'2022-02-02',crimerate,school))
...
def book_now(id,email):
    query_retrieve_book='select propertyID,property_type,location,city,address,price,description,availability,crime_rate,nearby_schools from property where propertyID=%s'
    mycursor.execute(query_retrieve_book,(id,))
    data=mycursor.fetchone()
    type=data[1]
    query_address='select * from renteraddress where emailID=%s'
...
def payment(id,email,type):
    if(request.method=="POST"):
        ...
        if(address=='new-address'):
            ...
            query_address_insert='insert into renteraddress(emailID,line_1,line_2,postal_code,city,state,country)values(%s,%s,%s,%s,%s,%s,%s)'
...
def rewards(email):
    ...
    if(rewards):
        return f'<h1>Currently You have "{str(rewards[1])}" points</h1>'
    else:
        return f'<h1>None</h1>'
